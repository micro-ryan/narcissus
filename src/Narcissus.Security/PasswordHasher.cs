...
namespace Narcissus.Security.Core
{
    public class PasswordHasher<TUser> : IPasswordHasher<TUser> where TUser:class
    {
        ...
        public Task<string> HashPassword(TUser user, string password)
        {
            return Task.FromResult(Convert.ToBase64String(HashPasswordV3(password, _rng)));
        }

        public Task<bool> VerifyHashedPassword(TUser user, string hashedPassword, string providedPassword)
        {
            ...
            switch (decodedHashedPassword[0])
            {
                ...
                default:
                    return Task.FromResult(false); // unknown format marker
            }
        }
